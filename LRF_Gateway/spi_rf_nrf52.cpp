...
uint8_t hal_spim_tx_rx (uint8_t * p_tx, uint32_t tx_len, uint8_t * p_rx, uint32_t rx_len)
{
    uint8_t buff[10];
    uint8_t l_len = tx_len + rx_len;
    memcpy (buff, p_tx, tx_len);



    // digitalWrite(ss_pin, 0);
    SPI.transfer (buff, l_len);
    // digitalWrite(ss_pin, 1);

    memcpy (p_rx, &buff[tx_len], rx_len);
    return buff[0];
...
rfStatus_t trx16BitRegAccess(uint8_t accessType, uint8_t extAddr, uint8_t regAddr, uint8_t *pData, uint8_t len)
{
    ...
    if((accessType&RADIO_READ_ACCESS) == RADIO_READ_ACCESS)  
    {
      ...
      readValue = hal_spim_tx_rx (tx_buff, 2, rx_buff, len);
      memcpy(pData, rx_buff, len);   
    }
    else if((accessType&RADIO_WRITE_ACCESS) == RADIO_WRITE_ACCESS)
    {
      memcpy (&tx_buff[2], pData, len);
      readValue = hal_spim_tx_rx (tx_buff, len+2, rx_buff, 0);
    }
...
{
	uint8_t rc;
    rc =  hal_spim_tx_rx (&cmd, 1, NULL, 0);
