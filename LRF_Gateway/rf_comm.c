...
uint32_t rf_comm_radio_init (rf_comm_radio_t * p_radio_params, rf_comm_hw_t * p_comm_hw)
{
    ...
    pinMode (g_comm_hw.rf_reset_pin, OUTPUT);
    digitalWrite (g_comm_hw.rf_reset_pin,1);
    delay (1);
    digitalWrite (g_comm_hw.rf_reset_pin,0);
    delay (1);
    digitalWrite (g_comm_hw.rf_reset_pin,1);
    //Set default config
    ...
    pinMode (g_comm_hw.rf_gpio0_pin, INPUT);
    pinMode (g_comm_hw.rf_gpio1_pin, INPUT);
    pinMode (g_comm_hw.rf_gpio2_pin, INPUT);
    pinMode (g_comm_hw.rf_gpio3_pin, INPUT);
    uint32_t irq_evt = 0;
    irq_evt = attachInterrupt(g_comm_hw.rf_gpio0_pin, rf_comm_gpiote_Handler, RISING);
    
#ifdef RF_COMM_AMPLIFIRE
    pinMode (g_comm_hw.rf_pa_pin, OUTPUT);
    pinMode (g_comm_hw.rf_lna_pin, OUTPUT);
...
//        //SerialMon.println("%s : %d %d (16bit)\n", __func__, srate_m, srate_e);
    ...
    else if (srate_m == (1<< 20))
    {
        ...
        trx8BitRegAccess (RADIO_WRITE_ACCESS, (0xFF & SYMBOL_RATE2), arr_reg, 1);
//        //SerialMon.println("%s : %d %d (20bit-0bit)\n", __func__, srate_m, srate_e);        
    }
    else 
    {
...
uint32_t rf_comm_set_pwr (int32_t pwr)
{
    ...
    //SerialMon.println("0x%x 0x%x 0x%x\n", arr_reg[0],arr_reg[1],arr_reg[2]);
    return 0;
...
#ifdef RF_COMM_AMPLIFIRE
    digitalWrite (g_comm_hw.rf_pa_pin,1);
#endif
...
{
#ifdef RF_COMM_AMPLIFIRE
    digitalWrite (g_comm_hw.rf_lna_pin, 1);
#endif
...
{
#ifdef RF_COMM_AMPLIFIRE
    digitalWrite (g_comm_hw.rf_lna_pin, 0);
    digitalWrite (g_comm_hw.rf_pa_pin, 0);
#endif
	/* Force transciever idle state */
	trxSpiCmdStrobe(SIDLE);
...
uint32_t rf_com_get_state ()
{
    uint8_t state;
    uint8_t random_buff;
	state = (uint8_t)trx16BitRegAccess(RADIO_READ_ACCESS , 0x2F, (0xFF & PARTNUMBER), &random_buff, 1);
    state &= 0x70;
    state >>= 4;
    return state;
